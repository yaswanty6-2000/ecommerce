...
const Wishlist = () => {
    ...
    const handleRemoveFromWishlist = (id: any) => {
        ...
        // You can use a refresh or use state to show updated data
    };
    ...
    if (wishlist.length === 0) {
        return (
            <>
                <Navbar />
                <Typography variant="h6" style={{ marginTop: '50px' }}>Your wishlist is empty.</Typography>
            </>
        )
    }
                    ...
                    {wishlist.map((product: any) => (
                        <Grid item xs={12} sm={6} md={4} key={product.id}>
                            <Card style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', padding: '16px' }}>
                                <img src={product.image} alt={product.name} style={{ height: '150px', width: '100%', objectFit: 'cover', marginBottom: '10px' }} />
                                <CardContent>
                                    ...
                                    <div style={{ display: 'flex', justifyContent: 'space-around', marginTop: '10px' }}>
                                        ...
                                        <Button variant="contained" color="error" onClick={() => handleRemoveFromWishlist(product.id)}>
                                            <Delete />
                                        </Button>
                                    </div>
                                </CardContent>
